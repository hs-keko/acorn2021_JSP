	...
	public List<TodoDto> getList(){
		...
		try {
			...
			while (rs.next()) {
				...
				// 현재 커서가 위치한 곳의 num, content, date 칼럼에 있는 데이터를 읽어와서 담기
				dto.setNum(rs.getInt("num"));
				dto.setContent(rs.getString("content"));
				dto.setRegdate(rs.getString("regdate"));
				// 하나의 할일 정보가 담긴 TodoDto 객체의 참조값을 List 에 누적시키기
				list.add(dto);
			}
		} catch (Exception e) {
			e.printStackTrace();
			...
			try {
				...
				if (pstmt != null)
					pstmt.close();
				if (conn != null)
					conn.close(); // close() 하면 자동으로 Connection Pool 에 반납됨
			} catch (Exception e) {
			}
		}
		// 할일 목록 리턴
